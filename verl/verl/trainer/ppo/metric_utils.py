...
def compute_reward_component_metrics(reward_extra_infos_dict: dict) -> dict[str, float]:
    """Return per-component reward means from the dict returned by a custom reward function.

    Any key other than ``score`` (the total reward, already tracked separately)
    is emitted as ``reward_components/<key>/mean``.
    """
    metrics = {}
