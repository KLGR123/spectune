...
- System prompt: `spectune.format.v1.SYSTEM_PROMPT` (default, injected by the artifact compiler). Several named versions live in `SYSTEM_PROMPT_REGISTRY` and are selectable via `--prompt` on `spectune.rollout`, `spectune.rollout.eval`, and `spectune.artifacts compile` (`basic` — no `<think>`/RDKit, `think` — `<think>` without RDKit, `think_rdkit` — default, `<think>` + RDKit notes).
...
`--tools` takes one or more space-separated tool names (e.g. `--tools nmr_generate nmr_repair`); omit it to use `DEFAULT_RL_TOOL_NAMES`. `--data-source` takes either one value (applied to every `--input` path) or exactly one value per `--input` path, so the NMRexp and "others" pools above can be tagged `spectune/nmrexp` / `spectune/others` while compiling in one run instead of two. `--prompt` selects which system prompt is baked into each row (default `think_rdkit`), the same switch that `spectune.rollout` accepts — pass it here so the compiled artifacts match the rollout prompt you trained against. Each row includes:
...
>Warning: `skills` now have conflicts with system prompt injection, use carefully.
...
```bash
bash /path/to/spectune/examples/run_grpo_*.sh
```
...
nohup bash run_grpo_wo_sft.sh > run.log 2>&1 &
tail -f run.log
```
