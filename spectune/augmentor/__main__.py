...
def _parse_information_mix(value: str) -> dict[str, float]:
    try:
        ...
        raise argparse.ArgumentTypeError(
            f"--information-mix must be a valid JSON object, e.g. "
            f"'{{\"none\":0.2,\"formula\":0.2,...}}': {exc}"
        ) from exc
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # Sampling
    p.add_argument("--dataset-name", default="nmrexp", metavar="NAME",
                   help="Source dataset name (default: nmrexp).")
    p.add_argument("--split", default="train", metavar="SPLIT",
                   help="Dataset split to draw from (default: train).")
    p.add_argument("--sample-size", type=int, default=0, metavar="N",
                   help="Number of rows to sample (0 = full split, default: 0).")
    ...
    # Information mix
    p.add_argument(
        "--information-mix", type=_parse_information_mix,
        metavar="JSON",
        default=None,
        help=(
    ...
    p.add_argument("--followup-probability", type=float, default=0.4, metavar="P",
                   help="Probability of adding information as a follow-up turn (default: 0.4).")

    # Construction
    p.add_argument("--max-fragment-items", type=int, default=2, metavar="N",
                   help="Max fragment hints per sample (default: 2).")
    p.add_argument("--max-reaction-items", type=int, default=2, metavar="N",
                   help="Max reaction context items per sample (default: 2).")

    # Noise
    p.add_argument("--nmr-noise-ratio", type=float, default=0.0, metavar="P",
                   help="Share of rows with corrupted NMR spectrum (default: 0.0).")
    p.add_argument("--nmr-noise-strength", type=float, default=0.3, metavar="S",
                   help="Corruption intensity in (0, 1] (default: 0.3).")
    p.add_argument("--formula-noise-ratio", type=float, default=0.0, metavar="P",
                   help="Share of formula-condition rows with corrupted formula (default: 0.0).")
    p.add_argument("--modal-drop-ratio", type=float, default=0.5, metavar="P",
                   help=(
    ...
    # LLM rewrite
    p.add_argument("--use-llm-rewrite", action="store_true",
                   help="Rewrite queries with an LLM (disabled by default).")
